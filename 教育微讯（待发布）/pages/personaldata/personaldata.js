...
Page({
  ...
  handleDesc: function() {
    ...
    wx.setStorageSync("desc", this.data.desc)
  },
  ...
  getUser: function() {
    var that = this;
    wx.request({
      url: app.globalData.url + 'api/user/user_info/?token=' + wx.getStorageSync('token'),
      method: 'post',
      header: {
        "content-type": "application/x-www-form-urlencoded"
      },
      success: function(res) {
        ...
        that.setData({
          ...
          phone: res.data.data.display_mobile,
  ...
  bindMultiPickerChange: function (e) {
    ...
    that.setData({
      ...
      province: that.data.multiArray[0][e.detail.value[0]]+'省',
      city: that.data.multiArray[1][e.detail.value[1]]+'市',
    })
    api.save_user_info({
      data: {
        province: that.data.multiArray[0][e.detail.value[0]],
